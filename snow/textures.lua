local vehicle_snowing_status = true

local VehicleSnow = { }
VehicleSnow.__index = VehicleSnow

local TIME = 25000 --Скорость снегу
...
addEventHandler ( "onClientPreRender", root,
	function ( )
		if vehicle_snowing_status then
			local now = getTickCount ( )
...
addEventHandler ( "onClientResourceStart", resourceRoot,
	function ( )
		VehicleSnow.texture = dxCreateTexture ( "single.fantastical" )
...
function SnowStatus(bool)
	vehicle_snowing_status = bool

	if bool then
		for k, v in pairs(getElementsByType("vehicle")) do
			snowedVehicles:add(v)
		end
	else
		for k, v in pairs(getElementsByType("vehicle")) do
			snowedVehicles:remove(v)
		end
	end
end