...
setBlipVisibleDistance(createBlip(Vector3(getElementPosition(getElementsByType('object',resourceRoot)[5])),44),400)
createBlip(2839,948,11, 56, 0, 0, 0, 255 )




setRadarAreaFlashing(createRadarArea(-2,1693,430,430,255,0,0,150),true)



local pozycje_zrzutu = {
	{291,2053,18 -1.5},
	{322,1800,18 -1.5},
	{109,1774,18 -1.5},
	{145,1920,20 - 1.5},
...
addEventHandler("onResourceStart", resourceRoot, function()
    for k, v in ipairs(getElementsByType("object", resourceRoot)) do 
        ...
        local lodObject = createObject(model, x, y, z, rx, ry, rz, true)
        setLowLODElement(v, lodObject)
    end
end)
...
function createBox()
	if getPlayerCount() < 5 then return end
	outputChatBox("=== Paczka spadła na strefe 51 ! ===",root,255,255,255)
	outputChatBox("=== W paczce znajduje się spora ilość przedmiotów ! ===",root,255,255,255)
	outputChatBox("=== Następna paczka: 30-60 min ! ===",root,255,255,255)
	local x,y,z = unpack(pozycje_zrzutu[math.random(#pozycje_zrzutu)])
	local obiect = createObject(964,x,y,z + 50)
	setElementDoubleSided(obiect,true)
	setElementAlpha(obiect,0)
	local object = createObject(2903,x,y,z + 50)
	setObjectScale(object,1)
	attachElements(object,obiect,0,0,7)
	moveObject(obiect,60000,x,y,z)
	setTimer(function(object,obiect)
		destroyElement(object)
		setElementAlpha(obiect,255)
	end,60100,1,object,obiect)
	local blip = createBlipAttachedTo(obiect,16)
	local tentCol = createColSphere(x,y,z,3)
	setTimer(function(tentCol,obiect,blip)
		destroyElement(tentCol)
		destroyElement(obiect)
		destroyElement(blip)
	end,60000*29,1,tentCol,obiect,blip)
	...
	setElementData(tentCol, "Coyote Backpack", math.random(1,3))
	setElementData(tentCol, "Alice Pack", math.random(3,5))
	setElementData(tentCol, "Toolbox", 10)
	setElementData(tentCol, "Ghillie Suit", 4)
	setElementData(tentCol, "Tent", 3)
	setElementData(tentCol, "GPS", 7)
	setElementData(tentCol, "Map", 7)
	setElementData(tentCol, "Tire", 3)
	setElementData(tentCol, "Engine", 2)
	setElementData(tentCol, "Civilian Clothing", 5)
	setElementData(tentCol, "Camouflage Clothing", 4)
	setElementData(tentCol, "Ghillie Suit [F]", 4)
	setElementData(tentCol, "Milk", 10)
	setElementData(tentCol, "Pizza", 10)
	setElementData(tentCol, "Burger", 10)
	setElementData(tentCol, "Soda Bottle", 10)
	setElementData(tentCol, "M4A1-S", 8)
	setElementData(tentCol, "G36C", 8)
	setElementData(tentCol, "AK-47", 8)
	setElementData(tentCol, "AWP", math.random(4,6))
	setElementData(tentCol, "M4A1-S Mag", math.random(8,12))
	setElementData(tentCol, "G36C Mag", math.random(8,12))
	setElementData(tentCol, "AWP Mag",math.random(6,10))
	setElementData(tentCol, "AK-47 Mag", math.random(12,15))
	setElementData(tentCol, "Ghillie Suit [F]", 5)
	setElementData(tentCol, "Military Backpack", math.random(3,5))
	...
	setElementData(tentCol, "First Aid Kit", 10)
	setElementData(tentCol, "Medic Bag", 5)
	setElementData(tentCol, "Bandage", 15)
	setTimer(createBox,3600000/2 + math.random(3600000/2),1)
end
setTimer(createBox,5000,1)
