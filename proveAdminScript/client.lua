...
function adminsOnline()
    ...
    outputChatBox("#c22d2d[#6b6b6b!#c22d2d]#FFFFFF ------ Administracja Online na służbie ------", 255, 255, 255, true)
    for k, v in pairs(AdminsTable) do
        outputChatBox("#6b6b6b● "..ColorsTable[v[1]], 255, 255, 255, true)
        local string = "  -   "
        statusAdminsOnline[v[1]] = false
        local number = true
        for c, g in pairs(getElementsByType("player")) do
            if getElementData(g, "duty") == v[1] then
                statusAdminsOnline[v[1]] = true
                if number then
        ...
        if statusAdminsOnline[v[1]] then
...
addEventHandler("onPlayerUseAdminRoom", root, function(player, status)
    if localPlayer ~= player then return end
