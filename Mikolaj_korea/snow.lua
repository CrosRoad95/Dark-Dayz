...
local settings = {type = "snow", density = 2000, wind_direction = {0.01,0.01}, wind_speed = 1, snowflake_min_size = 1, snowflake_max_size = 3, fall_speed_min = 1, fall_speed_max = 4, jitter = true}
				...
				--	outputDebugString(string.format("Flake removed. %.1f %.1f %.1f",flake.x,flake.y,flake.z))
					...
					if ground[gx][gy] and (flake.z+lz) > ground[gx][gy] then
						...
						if draw_x and draw_y then
							...
							dxDrawImageSection(draw_x, draw_y, flake.size, flake.size, flake.section[1], flake.section[2], 32, 32, "i/".. settings.type .. ".png", flake.rot, 0, 0, tocolor(222,235,255,flake.alpha))
							--	end
