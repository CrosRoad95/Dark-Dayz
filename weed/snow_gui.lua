...
settings = {type = "real", density = 10, wind_direction = {0.0,0.01}, wind_speed = 1, snowflake_min_size = 1, snowflake_max_size = 4, fall_speed_min = 1, fall_speed_max = 4}
...
function()
	...
	snow.window = guiCreateWindow(sx-290,sy-405,285,340,"Weed Settings",false)
	...
	snow.wind_direction_label = guiCreateLabel(13,117,123,19,"Weed wind direction:",false,snow.window)
	...
	snow.snow_type_label = guiCreateLabel(13,23,109,19,"Weed type:",false,snow.window)
	...
	snow.snow_density_label = guiCreateLabel(13,69,107,20,"Weed density:",false,snow.window)
		...
		snow.wind_speed_label = guiCreateLabel(13,218,123,20,"Weed wind speed:",false,snow.window)
	...
	snow.snowflake_size_label = guiCreateLabel(162,69,107,20,"Weed size:",false,snow.window)
	...
	snow.fall_speed_label = guiCreateLabel(13,265,123,20,"Weed fall speed:",false,snow.window)
...
function showSnowSettings()
	...
	--	end
		...
		outputChatBox("Weed settings are disabled.")

	end

end

addCommandHandler("weedsettings",showSnowSettings)
...
addCommandHandler("Weed Settings (hold shift)",showSnowSettingsBind)

--bindKey("w","down","Snow Settings (hold shift)")
...
local snowtype = xmlFindChild(file,"type",0)
	...
	if not snowtype then
 		snowtype = xmlCreateChild(file,"type")
 	end
...
local snowdensity = xmlFindChild(file,"density",0)
	...
	if not snowdensity then
 		snowdensity = xmlCreateChild(file,"density")
 	end
...
local snowwinddirection = xmlFindChild(file,"winddirection",0)
	...
	if not snowwinddirection then
 		snowwinddirection = xmlCreateChild(file,"winddirection")
 	end
...
local snowwindspeed = xmlFindChild(file,"windspeed",0)
	...
	if not snowwindspeed then
 		snowwindspeed = xmlCreateChild(file,"windspeed")
 	end
...
local snowflakesize = xmlFindChild(file,"snowflakesize",0)
	...
	if not snowflakesize then
 		snowflakesize = xmlCreateChild(file,"snowflakesize")
 	end
...
local snowfallspeed = xmlFindChild(file,"fallspeed",0)
	...
	if not snowfallspeed then
 		snowfallspeed = xmlCreateChild(file,"fallspeed")
 	end
...
local cursorhighlight = xmlFindChild(file,"highlightcursor",0)
	...
	if not cursorhighlight then
 		cursorhighlight = xmlCreateChild(file,"highlightcursor")
 	end
