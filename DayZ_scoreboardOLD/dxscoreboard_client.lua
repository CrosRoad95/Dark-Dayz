...
function doDrawScoreboard( rtPass, onlyAnim, sX, sY )
	if #scoreboardColumns ~= 0 then
		...
		--	textLength = dxGetTextWidth( "Polski Ser...", fontscale(sbFont, s(sbFontScale)), sbFont )
			...
			for key, column in ipairs( scoreboardColumns ) do
				...
				if sortBy.what == column.name then
					...
					local _, _, _, a = fromcolor( cHeader )
						dxDrawText( column.friendlyName or "-", topX+x+s(1), 	y+s(1), 	topX+x+s(1+column.width)-5, 	y+s(1)+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor( 0, 0, 0, a ), fontscale(columnFont, s(1)), columnFont, "left", "top", true, false, drawOverGUI )
						dxDrawText( column.friendlyName or "-", topX+x, 	y, 	topX+x+s(column.width)-5, 	y+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), cHeader, fontscale(columnFont, s(1)), columnFont, "left", "top", true, false, drawOverGUI )
				end
