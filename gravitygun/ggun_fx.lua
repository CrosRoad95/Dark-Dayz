...
function ggShellManage(PPlayer,PObject,isCreate)
	...
	if isCreate then
		if not shellTable.shader then
			...
			shellTable.texture = dxCreateTexture("textures/shell.jpg","dxt1")
...
addEventHandler("onGGShell",resourceRoot,ggShellManage)

---------------------------------------------------------------------------------------------------
-- bezier curve 
---------------------------------------------------------------------------------------------------
local bezierShader, bezierTexture, coronaShader = nil, nil, nil
local scX, scY = guiGetScreenSize()

function bezierCreate()
	if not bezierShader and not bezierTexture and not coronaShader then
		bezierShader = dxCreateShader("shaders/imgBezier.fx")
		coronaShader = dxCreateShader("shaders/imgCorona.fx")
		bezierTexture = dxCreateTexture("textures/line.jpg","dxt1")
		dxSetShaderValue(bezierShader,"sScrRes", scX, scY)
		dxSetShaderValue(coronaShader,"sScrRes", scX, scY)
		dxSetShaderValue(bezierShader,"sElementRotationZ", 0)
		dxSetShaderValue(bezierShader,"sTesselation", 20)
		dxSetShaderValue(bezierShader,"sWidth", 0.5)		
		dxSetShaderValue(bezierShader,"sFlipTexture",false)
		dxSetShaderValue(bezierShader,"sFlipVertex",false)
		dxSetShaderValue(bezierShader,"sIsBillboard",true)
		dxSetShaderValue(bezierShader,"sFogEnable",false)
		dxSetShaderValue(bezierShader,"fCullMode",1)
		dxSetShaderValue(bezierShader,"sTexColor",bezierTexture)
		dxSetShaderTessellation(bezierShader,1,20)
		dxSetShaderValue(coronaShader,"sLightAttenuation",0.7)
		dxSetShaderValue(coronaShader,"sLightAttenuationPower",6)
		dxSetShaderValue(coronaShader,"fDepthBias",0.05)
		dxSetShaderValue(coronaShader,"fCullMode",2)
		return true
	else
		return false
	end
...
function bezierDestroy()
	if bezierShader then
		destroyElement(bezierShader)
		bezierShader = nil
	end
	if bezierTexture then
		destroyElement(bezierTexture)
		bezierTexture = nil
	end
end

function drawBezier(x1,y1,z1,x2,y2,z2,x3,y3,z3,x4,y4,z4)
	if not bezierShader or not coronaShader or not bezierTexture then
		return
	end
	dxSetShaderValue(bezierShader,"sPointPosition1",x1,y1,z1)
	dxSetShaderValue(bezierShader,"sPointPosition2",x2,y2,z2)
	dxSetShaderValue(bezierShader,"sPointPosition3",x3,y3,z3)
	dxSetShaderValue(bezierShader,"sPointPosition4",x4,y4,z4)
	dxDrawImage(0,0,scX,scY,bezierShader,0,0,0,tocolor(255,153,0,255))
	dxSetShaderValue(coronaShader,"sLightPosition",x1,y1,z1) 
	dxDrawImage(0,0,scX,scY,coronaShader,0,0,0,tocolor(255,153,0,255))
	dxSetShaderValue(coronaShader,"sLightPosition",x4,y4,z4) 
	dxDrawImage(0,0,scX,scY,coronaShader,0,0,0,tocolor(255,153,0,255))
end
